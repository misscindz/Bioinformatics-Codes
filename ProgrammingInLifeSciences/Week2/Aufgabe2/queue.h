...
typedef struct {
  ...
  unsigned long enqueueindex,   /* points to entry into which element is to be
                                   enqueud */
                dequeueindex,   /* first element (head ) of queue */
...
/* The following function resizes the queue by doubling the space reservoir and
   making sure the queue is continious afterwards. */
